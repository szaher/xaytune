...
- **`examples/control_plane/05_train_and_evaluate.py`**: train, evaluate the trained model, stop at `DECIDING`.
...
- **Reported values change.** This is a correctness fix, not an API change: the functions, their arguments and the metric names are unchanged, and the earlier values were wrong. `compute_loss` and `compute_perplexity` take an optional `weights`; without it they average plainly, as before.
