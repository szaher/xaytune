# How trainlib Compares

The LLM fine-tuning ecosystem has several strong tools. This page explains where trainlib fits and when you might prefer one of the alternatives.

---

## At a Glance

| | trainlib | TRL | Axolotl | LLaMA-Factory | Unsloth | torchtune |
|---|---|---|---|---|---|---|
...
| **One-liner API** | :material-check: `trainlib.finetune(...)` | :material-close: | :material-close: | :material-close: | :material-close: | :material-close: |
...
## trainlib's Design Philosophy

trainlib is built around a principle that most tools only partially address: **the same library should serve both the quick experiment and the production pipeline, with the same config format and the same code path.**

```python
# 30 seconds to first result
trainlib.finetune(model="meta-llama/Llama-3.1-8B", dataset="data.jsonl", method="lora")
```
...
**Key difference:** TRL gives you a trainer class per algorithm (`SFTTrainer`, `DPOTrainer`, `GRPOTrainer`). trainlib gives you one function per recipe (`finetune()`, `align()`) and selects the algorithm via `method=`. This means less API surface to learn, but also less per-algorithm customization.
...
**Key difference:** Axolotl is YAML-first — you configure everything in YAML and run via CLI. trainlib treats Python and YAML as equal citizens: the same `TrainConfig` object drives both, and you can mix and match (load YAML, override in Python, pass to `trainer.train()`).
...
**Key difference:** LLaMA-Factory optimizes for the first-run experience — you can launch training from a browser in minutes. trainlib optimizes for the hundredth run — when you need reproducibility, extensibility, and integration with your own code.
...
**Key difference:** Unsloth is a speed optimizer — it makes training faster on a single GPU. trainlib is a workflow framework — it manages the full lifecycle from data to deployment. They solve different problems, and in fact Unsloth's optimizations can be used as a backend by other tools (LLaMA-Factory does this).
...
**Key difference:** torchtune gives you recipe scripts that you fork and modify. trainlib gives you a framework with extension points. torchtune says "here's a good starting point, make it yours." trainlib says "here's a system, extend it through the defined interfaces."
...
| **One tool from pre-training to deployment** | **trainlib** |
| **Python API + YAML + CLI + Web UI** | **trainlib** |
| **Extensible registries and callbacks** | **trainlib** |

trainlib's niche is the **full-lifecycle training library**: a single tool with a layered API (one-liners → config → full control) that covers pre-training, fine-tuning, alignment, evaluation, and export. If you only need one piece of that pipeline, a specialized tool might serve you better. If you want one cohesive system for all of it, that's what trainlib is for.
