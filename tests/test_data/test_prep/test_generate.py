...
class TestAugment:
    ...
    def test_augment_from_seeds(self, mock_call):
        ...
        result = generate(
            mode="augment", seed=seeds, n=2, format="alpaca",
    ...
    def test_augment_from_file(self, mock_call):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            result = generate(
                mode="augment", seed=str(path), n=1, format="alpaca",
...
class TestDistill:
    ...
    def test_distill_from_topic(self, mock_call):
        ...
        result = generate(
            mode="distill", topic="Kubernetes", n=3, format="alpaca",
            model="gpt-4o-mini", api_key="test-key",
        )
...
class TestEvolve:
    ...
    def test_evolve_increases_complexity(self, mock_call):
        ...
        result = generate(
            mode="evolve", seed=seeds, rounds=2, format="alpaca",
            model="test", api_key="test-key",
        )
        assert len(result.dataset) == 1
        assert mock_call.call_count == 2
...
class TestPostFilter:
    ...
    def test_post_filter_drops_short(self, mock_call):
        mock_call.side_effect = [
            ...
            json.dumps({"instruction": "Long question here", "output": "Long detailed answer here"}),
        ]
        result = generate(
            mode="augment", seed=[{"instruction": "X", "output": "Y"}], n=2,
            format="alpaca", model="test", api_key="k",
...
class TestAPIKeyResolution:
    ...
    def test_env_var_key(self, mock_call):
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "env-key"}):
            result = generate(
                mode="distill", topic="test", n=1, format="alpaca", model="test",
            )
            assert len(result.dataset) == 1
...
class TestReport:
    ...
    def test_report_counts(self, mock_call):
        mock_call.return_value = json.dumps({"instruction": "Q", "output": "A"})
        result = generate(
            mode="distill", topic="test", n=3, format="alpaca",
