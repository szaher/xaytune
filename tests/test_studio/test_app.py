...
from trainlib.cli import _build_parser  # noqa: E402
from trainlib.studio.app import _poll, build_config, create_app, validate_form  # noqa: E402
from trainlib.studio.jobs import JobInfo, JobManager, JobStatus  # noqa: E402


class TestValidateForm:
    def test_valid_inputs(self):
        errors = validate_form(
            recipe="finetune", method="full", model_name="m", data_path="d",
            quantization=None, learning_rate=2e-4, batch_size=4,
            warmup_steps=0, warmup_ratio=0.0, eval_split=0.1,
        )
        assert errors == []

    def test_empty_model_name(self):
        errors = validate_form(
            recipe="finetune", method="full", model_name="", data_path="d",
            quantization=None, learning_rate=2e-4, batch_size=4,
            warmup_steps=0, warmup_ratio=0.0, eval_split=0.0,
        )
        assert any("Model Name" in e for e in errors)

    def test_empty_data_path(self):
        errors = validate_form(
            recipe="finetune", method="full", model_name="m", data_path="",
            quantization=None, learning_rate=2e-4, batch_size=4,
            warmup_steps=0, warmup_ratio=0.0, eval_split=0.0,
        )
    ...
    def test_recipe_method_mismatch_align(self):
        errors = validate_form(
            recipe="align", method="full", model_name="m", data_path="d",
            quantization=None, learning_rate=2e-4, batch_size=4,
            warmup_steps=0, warmup_ratio=0.0, eval_split=0.0,
        )
    ...
    def test_recipe_method_mismatch_finetune(self):
        errors = validate_form(
            recipe="finetune", method="dpo", model_name="m", data_path="d",
            quantization=None, learning_rate=2e-4, batch_size=4,
            warmup_steps=0, warmup_ratio=0.0, eval_split=0.0,
        )
    ...
    def test_recipe_method_mismatch_pretrain(self):
        errors = validate_form(
            recipe="pretrain", method="lora", model_name="m", data_path="d",
            quantization=None, learning_rate=2e-4, batch_size=4,
            warmup_steps=0, warmup_ratio=0.0, eval_split=0.0,
        )
    ...
    def test_qlora_requires_4bit(self):
        errors = validate_form(
            recipe="finetune", method="qlora", model_name="m", data_path="d",
            quantization=None, learning_rate=2e-4, batch_size=4,
            warmup_steps=0, warmup_ratio=0.0, eval_split=0.0,
        )
    ...
    def test_qlora_with_4bit_passes(self):
        errors = validate_form(
            recipe="finetune", method="qlora", model_name="m", data_path="d",
            quantization="4bit", learning_rate=2e-4, batch_size=4,
            warmup_steps=0, warmup_ratio=0.0, eval_split=0.0,
        )
    ...
    def test_warmup_conflict(self):
        errors = validate_form(
            recipe="finetune", method="full", model_name="m", data_path="d",
            quantization=None, learning_rate=2e-4, batch_size=4,
            warmup_steps=100, warmup_ratio=0.03, eval_split=0.0,
        )
    ...
    def test_negative_learning_rate(self):
        errors = validate_form(
            recipe="finetune", method="full", model_name="m", data_path="d",
            quantization=None, learning_rate=-1e-4, batch_size=4,
            warmup_steps=0, warmup_ratio=0.0, eval_split=0.0,
        )
    ...
    def test_zero_batch_size(self):
        errors = validate_form(
            recipe="finetune", method="full", model_name="m", data_path="d",
            quantization=None, learning_rate=2e-4, batch_size=0,
            warmup_steps=0, warmup_ratio=0.0, eval_split=0.0,
        )
    ...
    def test_eval_split_out_of_range(self):
        errors = validate_form(
            recipe="finetune", method="full", model_name="m", data_path="d",
            quantization=None, learning_rate=2e-4, batch_size=4,
            warmup_steps=0, warmup_ratio=0.0, eval_split=1.5,
        )
        assert any("Eval Split" in e for e in errors)

    def test_multiple_errors_at_once(self):
        errors = validate_form(
            recipe="align", method="full", model_name="", data_path="",
            quantization=None, learning_rate=-1, batch_size=0,
            warmup_steps=10, warmup_ratio=0.1, eval_split=2.0,
        )
        assert len(errors) >= 5
...
class TestBuildConfig:
    def test_minimal(self):
        config = build_config(
            recipe="finetune", method="full",
            model_name="test-model", data_path="data.jsonl", data_format="alpaca",
        )
    ...
    def test_full_fields(self):
        config = build_config(
            recipe="pretrain", method="full",
            model_name="my-model", data_path="train.jsonl", data_format="pretrain",
            max_seq_length=4096, packing=False,
            batch_size=8, gradient_accumulation=4, learning_rate=1e-5,
            num_epochs=2, max_steps=1000, warmup_steps=100,
            scheduler="linear", weight_decay=0.1, max_grad_norm=0.5,
            mixed_precision="fp16", seed=123,
            eval_every_n_steps=200, early_stopping_patience=3,
            log_every_n_steps=5, output_dir="my-output",
        )
    ...
    def test_new_fields(self):
        config = build_config(
            recipe="finetune", method="qlora",
            model_name="m", data_path="d", data_format="alpaca",
            quantization="4bit", dtype="float16", trust_remote_code=True,
            source="huggingface", streaming=True, eval_split=0.1,
            warmup_ratio=0.03,
            checkpoint_every_n_steps=100, save_last=False,
            activation_checkpointing=True, async_checkpoint=True,
    ...
    def test_method_params_passed(self):
        config = build_config(
            recipe="align", method="dpo",
            model_name="m", data_path="d", data_format="preference",
...
class TestPollJob:
    ...
    def test_accumulates_metrics(self):
        mgr = JobManager()
        job = JobInfo(
            job_id="j1", status=JobStatus.RUNNING, recipe="finetune",
            created_at=0.0, started_at=time.time(),
    ...
    def test_shows_error(self):
        mgr = JobManager()
        job = JobInfo(
            job_id="j1", status=JobStatus.FAILED, recipe="finetune",
    ...
    def test_shows_status_badge(self):
        mgr = JobManager()
        job = JobInfo(
            job_id="j1", status=JobStatus.COMPLETED, recipe="finetune",
            created_at=0.0,
        )
        mgr._jobs["j1"] = job
        status, fig, metrics, history = _poll(mgr, "j1", [])
...
class TestStudioCLI:
    ...
    @patch.dict("sys.modules", {"trainlib.studio.server": None})
