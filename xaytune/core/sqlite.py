"""SQLite connection setup and the write-transaction boundary (ADR-005 §8).

In the core rather than beside the control-plane schema because a runtime
backend needs a durable registry of its own (ADR-013 §2) and must not reach
back into the control plane's persistence layer to get one. The dependency
runs one way: the control plane drives runtimes, so a runtime that imported
``xaytune.storage`` would invert it -- and every worker process would pay to
import a write surface it is not allowed to use.
...
SQLite serializes write transactions and revision CAS
(:mod:`xaytune.storage.repository`) protects aggregate-level concurrency;
together those are the guarantee. ADR-004's controller lease is a different
concern -- it stops two controllers making duplicate *control decisions*, which
no amount of database locking can detect, because both writes would be
individually valid.
"""
